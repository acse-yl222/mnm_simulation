...
def solve_NS(u, v, p, rho, nu, courant, dt_min, t_end, dx, dy, C,k,D, rtol=1.e-5, logs=False, outint=100):
    ...
    while t < t_end:
        ...
        if logs and time_it % outint == 0:
            velocity = np.max(np.sqrt(u ** 2 + v ** 2))
            N1 = D * dt / dx ** 2
            N2 = dx * velocity / D
            N3 = dx ** 2 * k / D
        ...
        v_old = v
        v = temp
    return u, v, p,N1,N2,N3


start = time.time()
u, v, p,N1,N2,N3= solve_NS(u, v, p, rho, nu, courant, dt_min, t_end, dx, dy, C,k,D, rtol=1.e-5, logs=True, outint=1000)
